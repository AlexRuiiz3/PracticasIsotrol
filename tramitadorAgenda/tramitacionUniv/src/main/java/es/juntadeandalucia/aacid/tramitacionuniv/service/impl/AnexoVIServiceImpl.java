...
import es.juntadeandalucia.aacid.tramitacionuniv.pojo.anexovi.AnexoVI;
import es.juntadeandalucia.aacid.tramitacionuniv.service.IAnexoVIService;

public class AnexoVIServiceImpl implements IAnexoVIService {

  /** interfaz de solicitudes */
  private ISolicitudDao solicitudDao;

  private static final Log LOG = LogFactory.getLog(AnexoVIServiceImpl.class);

  @Override
  @Transactional
  public void procesamientoEspecifico(BigDecimal idExp, Long idSolicitud, EntregaVeaType ultimoAnexoVI)
    ...
    if (ultimoAnexoVI != null) {

      anexo = new AnexoVI();
      anexo.setIdSolicitud(idSolicitud);

      final Class<?> clazz = Class.forName("es.juntadeandalucia.aacid.tramitacionuniv.pojo.anexovi.AnexoVI");

      // Marcamos que el valor es nulo si la cadena es "-", "null", "" o " "
      final List<CampoVeaType> camposVea = ultimoAnexoVI.getFormularios().getFormulario().get(0).getCampo().stream().map(campoVea -> {
