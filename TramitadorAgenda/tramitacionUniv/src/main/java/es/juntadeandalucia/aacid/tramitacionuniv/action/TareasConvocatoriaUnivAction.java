 ...
 *
 */
public class TareasConvocatoriaUnivAction extends BaseTareasAction {
  ...
  private final transient Logger log = Logger.getLogger(TareasConvocatoriaUnivAction.class);
   ...
   *           exception
   */
  public String guardarDatosConvocatoriaUniv() throws ParseException, BusinessException {
    String idExp = null;
  ...
  /**
   * Funcion que redirige a la pantalla del listado de los expedientes hijos de una convocatoria Universidad.
   *
   * @return success or error
   *
   **/
  ...
  /**
   * Funcion que devuelve la lista los expedientes hijos de una convocatoria de universidad para mostrar en la vista.
   *
   * @return success or error
   * @throws ParseException
   * @throws BusinessException
   ...
   *           exception
   */
  public String cargarListadoExpedientesHijosConvocatoriaUniv() {
    try {
      listaExpedientesHijosConvocatoriaDTO = new ArrayList<>();
      user = getUsuarioSesion();
      final String idExp = user.getExpediente().getRefExpediente();

      log.debug("Se va a proceder a listar cada uno de los expedientes hijos, para el padre: " + idExp);
      final List<TrExpediente> listaExpedientesHijosConvocatoria = trewaService.obtenerExpedientesHijos(new TpoPK(idExp),
          ConstantesTramitacion.FASE_CONV_PRESENTACION);
      ...
      for (final TrExpediente exp : listaExpedientesHijosConvocatoria) {
        ...
        final SolicitudDatosGeneralesDTO solicitudDatosGeneralesDTO = solicitudService.datosGeneralesSolicitudByIdExpTrewa(exp.getREFEXP().toString());
        ...
        if (inteExp == null || inteExp.getINTERESADO() == null) {
          ...
          univ = inteExp.getINTERESADO().getNOMBRE() + " " + inteExp.getINTERESADO().getAPELLIDO1() + " " + inteExp.getINTERESADO().getAPELLIDO2();
  ...
  public String cargarListadoExpedientesProvConceDesest() {
    try {
      ...
      final List<TrExpediente> listaExpedientesHijoTrewa = trewaService.obtenerExpedientesHijos(new TpoPK(idExp), null);

      if (CollectionUtils.isNotEmpty(listaExpedientesHijoTrewa)) {
        listaExpedientesHijosConvocatoriaDTO = solicitudService.datosExpedienteProvConceDesest(listaExpedientesHijoTrewa);
  ...
  public String cargarListadoExpedientesProvConce() {
    try {
      ...
      listaSolicitudConcesionDTO = solicitudConcesionService.obtenerBeneficiariasSuplentes(idExp, fases, finalidad, false);
      return SUCCESS;
    } catch (final Exception e) {
      addActionError(e.getMessage());
      log.error(e.getMessage(), e);
