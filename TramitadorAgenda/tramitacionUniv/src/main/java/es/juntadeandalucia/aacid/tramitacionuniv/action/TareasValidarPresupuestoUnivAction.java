...
public class TareasValidarPresupuestoUnivAction extends BaseTareasAction {
  ...
  private final transient Logger log = Logger.getLogger(TareasValidarPresupuestoUnivAction.class);
  ...
  public String envioEmailDocumentoPresupuestoValidadoUniv() {
    user = getUsuarioSesion();
    try {
      ...
      log.error("Error en el envio de correo automatico con el documento de presupuesto validado adjunto. Causa: " + e.getMessage(), e);
      addActionError(e.getMessage());
      return ERROR;
    } catch (final Exception ex) {
      log.error("Error en el envio de correo automatico con el documento de presupuesto validado adjunto. Causa: " + ex.getMessage(), ex);
  ...
  public String envioEmailDocumentoBeneficiarosSuplentesUniv() {
    user = getUsuarioSesion();
    try {
      final SolicitudDTO solicitudDTO = solicitudService.obtenerSolicitudByIdExp(user.getExpediente().getRefExpediente());
      envioEmailAutomaticoService.enviarEmail(ConstantesTramitacion.VALIDACION_PRESUPUESTO, solicitudDTO.getEmail(), user.getExpediente().getRefExpediente(),
  ...
  public String envioEmailDocumentoInformeValoracionUniv() {
    user = getUsuarioSesion();
    try {
      final SolicitudDTO solicitudDTO = solicitudService.obtenerSolicitudByIdExp(user.getExpediente().getRefExpediente());
      envioEmailAutomaticoService.enviarEmail(ConstantesTramitacion.VALIDACION_PRESUPUESTO, solicitudDTO.getEmail(), user.getExpediente().getRefExpediente(),
          ConstantesTramitacion.ETIQUETA_INF_VALORACION_UNIV, null);
    } catch (final AddressException e) {
