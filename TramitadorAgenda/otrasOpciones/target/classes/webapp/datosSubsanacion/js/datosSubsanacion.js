...
$(function() {
  var type = "POST";
  var url = 'buscarSolicitudesSeleccionadas.action';
  var success = function(data) {
    $("#fechaRequerimientoSubsanacion").val(data.fechaSubsanacion);
    $("#fechaLimiteSubsanacion").val(data.fechaLimite);
    $("#fechaEntregaSubsanacion").val(data.fechaEntrega);
    $("#fechaRegistroSubsanacion").val(data.fechaRegistro);
    $("#lugarRegistroSubsanacion").val(data.lugarRegistro);

    inicializarDatepickers(["fhEntregaSubsanacion", "fhRegistroSubsanacion"], "dd/mm/yyyy");

    if (data.catalogosSeleccionados !== null) {
...
function cargarSelecGrupoSubsanacion() {
  $('#tablaGrupoSubsanaciones').DataTable({
    ...
    "autoWidth": false,
    "ajax": {
      "url": "buscarTipoCatalogo.action",
      "data": {
        "tipoProcedimiento": $("#tipoProcedimiento").val()
      },
      "dataType": 'json',
...
function guardarSubsanacion() {
  if ($("#formDatosSubsanacion").valid()) {
    ...
    var enviar = {
      listadoSubsanaciones: catalogoGuardar,
      fechaEntrega: $('#fechaEntregaSubsanacion').val(),
      fechaRegistro: $('#fechaRegistroSubsanacion').val(),
      tipoProcedimiento: $("#tipoProcedimiento").val(),
      lugarRegistro: $('#lugarRegistroSubsanacion').val(),
    };

    var type = "POST";
    var url = 'guardarSubsanacionesValorador.action';
    var data = enviar;
    var success = function(data) {
      $("#messagesAndErrorsSubsanacion").html(data);
      $("#messagesAndErrorsSubsanacion").removeClass('d-none');
      if (listaSubsanacionesSeleccionadas.length === 0) {
        $('#tablaSubsanacionesSeleccionadas').DataTable().destroy();
        $('#tablaSubsanacionesSeleccionadas').addClass('d-none');
        $('#mensajeInfoSubsanacionSeleccioanda').removeClass('d-none');
    ...
    var error = function(data) {
      $("#messagesAndErrorsSubsanacion").html(data.responseText);
      $("#messagesAndErrorsSubsanacion").removeClass('d-none');
      if (listaSubsanacionesSeleccionadas.length === 0) {
        $('#tablaSubsanacionesSeleccionadas').DataTable().destroy();
        $('#tablaSubsanacionesSeleccionadas').addClass('d-none');
