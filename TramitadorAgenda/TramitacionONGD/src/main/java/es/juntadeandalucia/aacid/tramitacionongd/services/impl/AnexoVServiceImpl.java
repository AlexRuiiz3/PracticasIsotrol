...
import es.juntadeandalucia.aacid.tramitacionongd.pojo.anexov.AnexoV;
import es.juntadeandalucia.aacid.tramitacionongd.pojo.anexov.DocumentoAdministracionJA;
import es.juntadeandalucia.aacid.tramitacionongd.pojo.anexov.DocumentoOtraAdministracion;
 ...
 *
 */
public class AnexoVServiceImpl implements IAnexoVService {

  /** interfaz de solicitudes */
  private ISolicitudDao solicitudDao;

  private static final Log LOG = LogFactory.getLog(AnexoVServiceImpl.class);

  @Override
  @Transactional
  public void procesamientoEspecifico(final BigDecimal idExp, final Long idSolicitud, final EntregaVeaType ultimoAnexoV) throws ClassNotFoundException,
      ParseException, IllegalAccessException, NoSuchFieldException, ParseCamposVeaException, AlegacionesAceptaReformulaException {

    AnexoV anexo = null;

    if (ultimoAnexoV != null) {

      anexo = new AnexoV();
      anexo.setIdSolicitud(idSolicitud);

      final Class<?> clazz = Class.forName("es.juntadeandalucia.aacid.tramitacionongd.pojo.anexov.AnexoV");

      // Marcamos que el valor es nulo si la cadena es "-", "null", "" o " "
      final List<CampoVeaType> camposVea = ultimoAnexoV.getFormularios().getFormulario().get(0).getCampo().stream().map(campoVea -> {
      ...
      // ----------------------------------------------------------------------------------------------------
      solicitud.setTxNotifemailautlugar(anexo.getNotifEmailAutLugar());
      solicitud.setTxNotiftelefonoautlugar(anexo.getNotifTelefonoAutLugar());
      ...
      // ----------------------------------------------------------------------------------------------------
      solicitud.setTxDatostitulo(anexo.getTituloProyecto());
      ...
      // PRESENTADOS
      solicitud.setLgDoc1(anexo.getDocu1());
      solicitud.setLgDoc2(anexo.getDocu2());
      solicitud.setLgDoc3(anexo.getDocu3());
      solicitud.setLgDoc4(anexo.getDocu4());
      ...
      solicitudDao.guardarAlegacionesAceptaReformulaONGD(solicitud);
  ...
  private List<CampoVeaType> procesarCampos(final List<CampoVeaType> camposVea, final AnexoV solicitud) throws ParseException {
