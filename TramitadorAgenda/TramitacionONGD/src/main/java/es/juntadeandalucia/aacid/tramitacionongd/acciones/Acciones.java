package es.juntadeandalucia.aacid.tramitacionongd.acciones;
...
import es.juntadeandalucia.aacid.tramitacionongd.services.IAnexoVService;
import es.juntadeandalucia.aacid.tramitacionongd.ws.client.IntegrationClientTramitadorViejo;
...
public class Acciones extends TrAccesoUI {
  ...
  private static IAnexoIVServiceOngd anexoIVService;
  private static IAnexoVService anexoVService;
  private static IAnexoVIIService anexoVIIService;
  ...
  static {
    ...
    anexoIVService = (IAnexoIVServiceOngd) context.getBean("anexoIVServiceOngd");
    anexoVService = (IAnexoVService) context.getBean("anexoVService");
    anexoVIIService = (IAnexoVIIService) context.getBean("anexoVIIService");

  }

  public final Integer invocarWsTramitadorViejo(final BigDecimal idExp) throws TramitacionException, IOException {
    Integer resultado = 0;
    try {

      final IntegrationClientTramitadorViejo client = new IntegrationClientTramitadorViejo();

      final String response = client.obtenerRespuestaPeticionAacidViejo(idExp.toPlainString());
      if (response != null) {
        resultado = 1;
        return resultado;
      }
    } catch (final TramitacionException e) {
      throw new TramitacionException("Error al invocar al servicio web del tramitador", e);
    }
    return resultado;
  }
  ...
  public final Integer aniadirDesistimientoEntidad(final BigDecimal idExp) {
    ...
    try {
      ...
      if (expedientes.length > 0) {
        ...
        final Long anio = (long) fechaCreacion.get(Calendar.YEAR);
        final Map<String, Long> mapaExclusiones = ConstantesTramitacionONGD.obtenerMapaExclusionesComunes();
        final List<ExclusionDTO> exclusiones = exclusionService.obtenerExclusionesDTOByOrdenTipo(ConstantesTramitacion.TIPO_CONV_ONGD,
            ConstantesTramitacion.EXCLUSION_TIPO_ADMISION, mapaExclusiones.get(ConstantesTramitacionONGD.COMUNES), anio, null);

        final Optional<ExclusionDTO> exclusion = exclusiones.stream().filter(s -> ConstantesTramitacionONGD.EXCLUSION_DESISTIMACION.equals(s.getDescripcion()))
            .findFirst();
  ...
  public final Integer recepcionSubsanacionDesdeVea(final BigDecimal idExp) {
    ...
    try {
      ...
      final TrFaseActualExpediente faseActual = fasesActual.stream()
          .filter(f -> ConstantesTramitacionONGD.FASE_COMPROBACION_SUBSANACION.equals(f.getFASE().getDESCRIPCION())).findAny().orElse(null);
      if (faseActual != null) {
        ...
        if (datosVea != null && CollectionUtils.isNotEmpty(datosVea.getEntrega())) {
          ...
          final EntregaVeaType ultimaEntregaSubsanacion = listaEntregas.stream()
              .filter(s -> ConstantesTramitacionONGD.TAREA_WEB_SUBSANACION.equals(s.getIdTareaTrewa())).findFirst().orElse(null);
        ...
        LOG.error("El expediente se encuentra en la fase " + fasesActual + ". El expediente debe estar en la fase "
            + ConstantesTramitacionONGD.FASE_COMPROBACION_SUBSANACION);
      }
      ...
      LOG.error("Se ha producido un error al recuperar la solicitud mientras procesabamos la subsanción para el expediente con id " + idExp.toString(), e);
  ...
  /**
   * Método que obtiene el ultimo FORMULARIO DE ALEGACIONES/ACEPTACIÓN/REFORMULACIÓN Y PRESENTACIÓN DE DOCUMENTOS y lo guarda en el sistema
   *
   * @param idExp
   *          Identificador del expediente. @return Indica si se ha procesado correctamente o no. @throws AlegacionesException @throws
   */
  public final int recepcionAlegacionTrasExclDesdeVea(final BigDecimal idExp) throws AlegacionesException {
    ...
    try {
      ...
      if (datosVea != null && CollectionUtils.isNotEmpty(datosVea.getEntrega())) {
        ...
        // Recuperamos la primera entrega, que gracias al reverse serÃ¡ el Ãºltimo FORMULARIO DE ALEGACIONES Y
        // PRESENTACIÃ“N DE DOCUMENTOS
        final EntregaVeaType ultimoAnexoIV = entregas.stream() // s
            .filter(s -> ConstantesTramitacionONGD.ID_TAREA_TREWA_ALEG_TRAS_EXCL.equals(s.getIdTareaTrewa())) //
            .findFirst() //
            .orElse(null);

        // Una vez tenemos la Ãºltima entrega del FORMULARIO DE ALEGACIONES Y PRESENTACIÃ“N DE DOCUMENTOS
        // comenzamos
      ...
      LOG.error("Se ha producido un error al leer el xml de recepciÃ³n del FORMULARIO DE ALEGACIÃ“N A LA SOLICITUD DE SUBVENCIÃ“N para el expediente con id "
          + idExp.toString(), e);
    } catch (final TramitacionException e) {
      LOG.error(
          "Se ha producido un error al recuperar la solicitud mientras procesabamos la recepciÃ³n del FORMULARIO DE ALEGACIÃ“N A LA SOLICITUD DE SUBVENCIÃ“N para el expediente con id "
              + idExp.toString(),
          e);
    } catch (final ClassNotFoundException e) {
      ...
      LOG.error("Se ha producido un error al setear algÃºn campo de VeA", e);
    }
  ...
  /**
   * MÃ©todo que obtiene el ultimo FORMULARIO DE ALEGACIONES/ACEPTACIÃ“N/REFORMULACIÃ“N Y PRESENTACIÃ“N DE DOCUMENTOS y lo guarda en el
   * sistema
   *
   * @param idExp
   ...
   */
  public final int recepcionAlegacionesAceptaReformulaDesdeVea(final BigDecimal idExp) throws AlegacionesAceptaReformulaException {
    ...
    try {
      ...
      if (datosVea != null && CollectionUtils.isNotEmpty(datosVea.getEntrega())) {
        ...
        final EntregaVeaType ultimoAnexoV = entregas.stream() // s
            .filter(s -> ConstantesTramitacionONGD.ID_TAREA_TREWA_RECEP_ALEG_ACEPT_REFORM.equals(s.getIdTareaTrewa())) //
            .findFirst() //
            .orElse(null);
        ...
        // con el procesamiento de los campos.
        anexoVService.procesamientoEspecifico(idExp, solicitudDTO.getIdSolicitud(), ultimoAnexoV);
   ...
   */
  public final int recepcionComunicacionInicioDesdeVea(final BigDecimal idExp) throws RecepcionComunicacionException {
    ...
    try {
      ...
      if (datosVea != null && CollectionUtils.isNotEmpty(datosVea.getEntrega())) {
        ...
        // Recuperamos la primera entrega, que gracias al reverse será el último ANEXO VII: FORMULARIO DE COMUNICACIÓN DE INICIO
        final EntregaVeaType ultimoAnexoVII = entregas.stream()
            .filter(s -> ConstantesTramitacionONGD.ID_TAREA_TREWA_RECEPCION_COMUNICACION_INICIO.equals(s.getIdTareaTrewa())).findFirst().orElse(null);

        // Una vez tenemos la última entrega del ANEXO VII: FORMULARIO DE COMUNICACIÓN DE INICIO comenzamos con el procesamiento de los
        // campos.
        anexoVIIService.procesamientoEspecifico(idExp, solicitudDTO.getIdSolicitud(), ultimoAnexoVII);
  ...
  private AaciTSolicitudsubongd procesamientoCamposFormularioVeA(final BigDecimal idExp, final SolicitudDTO solicitudDTO, final DatosVea datosVea,
    ...
    if (ultimaEntregaSubsanacion != null) {
      ...
      solicitudService.guardarSolicitudSubsanacion(solicitudSubsanacion, camposPaises, ConstantesTramitacion.TIPO_CONV_ONGD);
    } else {
      LOG.error("No se han encontrado formulario de subsanción para el expediente con id " + idExp);
    }
