...
public class TareasConcesionDefinitivaAction extends BaseTareasAction {
  ...
  private static final String MENSAJE_EXITO_GEN_INFORME_LISTADO_CONCESION_DEFINITIVA = "Se ha generado correctamente el informe del listado de concesi\u00F3n definitiva";
  private transient IEnvioEmailAutomaticoService envioEmailAutomaticoService;

  public String generarDocumentoListadoResolucionDefinitiva() {
    ...
    service.submit(() -> {
      try {
        ...
        final Map<String, Object> datos = concesionDefinitivaService.generarListadoConcesionDefinitiva(user, false);
        DocumentoExpedienteDTO doc = tareasService.generarInformes(datos, ConstantesTramitacion.PLANTILLA_LISTADO_CONCESION_DEFINITIVA_ONGD,
            ConstantesTramitacion.NOMBRE_INFORME_SALIDA_LISTADO_CONCESION_DEFINITIVA_ONGD, ConstantesTramitacion.NOMBRE_DOC_LIST_RESOLUCION_DEFINITIVA_ONGD,
            ConstantesTramitacion.NOMBRE_TAREA_GENERAR_INFORME_LISTADO_CONCESION_DEFINITIVA_ONGD, user);
  ...
  public String generarDocumentoResolucionDefinitiva() {
    user = getUsuarioSesion();
    try {
      final Map<String, Object> datos = concesionDefinitivaService.generarConcesionDefinitiva(user, false);

      DocumentoExpedienteDTO doc = tareasService.generarInformes(datos, ConstantesTramitacion.PLANTILLA_RELACION_CONCESION_DEFINITIVA_ONGD,
          ConstantesTramitacion.NOMBRE_INFORME_SALIDA_CONCESION_DEFINITIVA_ONGD, ConstantesTramitacion.NOMBRE_DOC_RESOLUCION_DEFINITIVA_ONGD,
          ConstantesTramitacion.NOMBRE_TAREA_GENERAR_INFORME_CONCESION_DEFINITIVA_ONGD, user);
  ...
  public String envioEmailDocumentoConcesionDefinitiva() {
    user = getUsuarioSesion();
    try {
      ...
      envioEmailAutomaticoService.enviarEmail(ConstantesTramitacion.CAUSA_CONCESION_DEFINITIVA, solicitudDTO.getEmail(),
          user.getExpediente().getRefExpediente(), ConstantesTramitacion.ETIQUETA_DOC_CON_DEF_ONGD, null);
